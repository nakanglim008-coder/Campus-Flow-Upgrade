    ...
    <StudentLayout>
      <div className="space-y-6">
        ...
        {!loading && exeats.length === 0 && (
          <div className="glass-card rounded-2xl p-12 text-center text-[var(--color-muted-foreground)]">
          ...
          {exeats.map((e) => {
            ...
            return (
              <div key={e.id} className="glass-card rounded-2xl p-4 space-y-3">
                ...
                {e.rejectReason && (
                  <p className="text-xs text-[var(--color-destructive)] pl-12 bg-[oklch(0.6_0.22_25_/_0.08)] rounded-lg px-3 py-2">
                    Reason: {e.rejectReason}
                  </p>
                )}
                {canQr && (
                  <div className="pl-12">
                    <button onClick={() => setShowQr(e)} className="text-xs text-[var(--color-primary)] font-medium hover:underline flex items-center gap-1">
                      View QR Pass
                    </button>
                  </div>
                )}
                <p className="text-xs text-[var(--color-muted-foreground)] pl-12">{new Date(e.createdAt).toLocaleDateString()}</p>
              </div>
            );
          })}
        </div>
      ...
      {showQr && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 px-4" onClick={() => setShowQr(null)}>
          <div className="glass-card rounded-3xl p-8 flex flex-col items-center gap-5 max-w-xs w-full" onClick={(e) => e.stopPropagation()}>
            ...
            <button onClick={() => setShowQr(null)} className="w-full py-2.5 rounded-xl bg-[var(--color-secondary)] text-sm font-medium">Close</button>
          </div>
        </div>
      )}
    </StudentLayout>
